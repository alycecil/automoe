...
function actualFriendsDo(hits) {
	...
	if (window.location.pathname == "/give/end.php") {
		...
		if (hasData(other) && window.location.pathname == "/friend/list.php") {
			...
			other[0].click();
		} else if (hasData(allMyFriends)
				&& window.location.pathname == "/friend/list.php") {
			if (hits > quitCount) {
				setFriendsData(0);
				done();
			}
...
function letsNap() {
	chrome.storage.local.get('lastNap' + prefix, function(items) {
		...
		if (items['lastNap' + prefix] == null
				|| items['lastNap' + prefix] + napTime * 2 < now) {
			var data = {};
	...
	if (request.tabid) {
		...
		if (!postedTimes) {
			chrome.storage.local.get('task' + prefix, function(items) {
				$('body').prepend(
						"event=" + items['task' + prefix] + ", prefix="
								+ prefix + ", delay=" + time + ", fail="
								+ failTime + "<br>");
			});
			postedTimes = true;
		}
